...
export default function InsightsPage() {
  ...
  return <main className="insights-page mx-auto max-w-7xl pb-10">
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
      ...
      <div className="rounded-3xl border border-slate-200 bg-slate-950 p-6 text-white shadow-sm"><div className="flex items-center justify-between"><div><p className="text-sm font-bold">Ritmo de foco</p><p className="mt-1 text-sm text-white/60">Minutos por dia.</p></div><Clock3 className="text-teal-300" size={22} /></div><FocusChart values={metrics.focusMinutes} labels={metrics.dates.map(dateLabel)} dark={dark} reducedMotion={reducedMotion} /><p className="mt-5 text-3xl font-bold">{weeklyFocus}<span className="ml-1 text-sm font-medium text-white/60">min</span></p></div>
    </section>
